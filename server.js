...
app.post('/api/requisitions', async (req, res) => {
  try {
    ...
    // Create the requisition first to get the ID
    const response = await fetch(`${BASE_URL}/requisitions/`, {
      ...
      body: JSON.stringify({
        // We'll update the redirect URL after we get the requisition ID
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error('Requisition API error response:', errorText);
    ...
    // If the redirectUrl contains a placeholder for the requisitionId, replace it
    if (requisition.link && requisition.link.includes('REQUISITION_ID_PLACEHOLDER')) {
      requisition.link = requisition.link.replace('REQUISITION_ID_PLACEHOLDER', requisition.id);
      console.log('Updated redirect URL with requisitionId:', requisition.link);
    }
